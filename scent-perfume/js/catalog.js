...
function renderBrandFilter() {
  ...
  PRODUCTS_DATA.forEach(p => {
    if (!p.brand) return;
    if (!grouped[p.brand]) grouped[p.brand] = 0;
    grouped[p.brand]++;
  });

  const allBrands = Object.keys(grouped).sort((a, b) => a.localeCompare(b, 'ru'));
  const VISIBLE = 5;

  function renderList(filter = '') {
    const filtered = filter
      ? allBrands.filter(b => b.toLowerCase().includes(filter.toLowerCase()))
      : allBrands;

    const shown = filtered.slice(0, VISIBLE);
    const rest  = filtered.slice(VISIBLE);

    const items = (list) => list.map(brand => `
      <button class="brand-pill${state.filters.brands.includes(brand) ? ' brand-pill--active' : ''}"
              type="button" data-brand="${escapeHTML(brand)}">
        <span>${escapeHTML(brand)}</span>
        <span class="brand-pill__count">${grouped[brand]}</span>
      </button>`).join('');
    ...
    container.innerHTML = `
      <div class="brand-search-wrap">
        ...
        <input class="brand-search" id="brandSearchInput" placeholder="Найти бренд..." value="${escapeHTML(filter)}" autocomplete="off" />
      </div>
      <div class="brand-pill-list">
        ${items(shown)}
        ${rest.length ? `
          <div class="brand-pill-more" id="brandPillMore">
            ${items(rest)}
          </div>
          <button class="brand-show-more" id="brandShowMore" type="button">
            Ещё ${rest.length}
    ...
    // поиск
    const searchInput = container.querySelector('#brandSearchInput');
    if (searchInput) {
    ...
    // кнопка "ещё"
    const showMore = container.querySelector('#brandShowMore');
    const moreBlock = container.querySelector('#brandPillMore');
    if (showMore && moreBlock) {
      showMore.addEventListener('click', () => {
        moreBlock.classList.toggle('brand-pill-more--open');
        const open = moreBlock.classList.contains('brand-pill-more--open');
        showMore.innerHTML = open
          ? `Скрыть <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="18 15 12 9 6 15"/></svg>`
          : `Ещё ${rest.length} <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg>`;
      });
    }

    // клик по бренду
    container.querySelectorAll('.brand-pill').forEach(btn => {
...
function renderActiveTags() {
  ...
  state.filters.brands.forEach(brand => {
    tags.push({ label: brand, key: `brand_${brand}` });
  });
...
function removeTag(key) {
  if (key === 'search') {
    ...
    state.filters.brands = state.filters.brands.filter(b => b !== brand);
    const cb = document.querySelector(`input[name="brand"][value="${CSS.escape(brand)}"]`);
    if (cb) cb.checked = false;
  }
  renderCatalog();
}
...
function resetAllFilters() {
  state.filters = { search: '', category: 'all', maxPrice: 200000, scentTypes: [], brands: [] };
  state.sort = 'default';
...
function bindFilterEvents() {
  ...
  document.querySelectorAll('input[name="brand"]').forEach(cb => {
    cb.addEventListener('change', () => {
      if (cb.checked) {
        if (!state.filters.brands.includes(cb.value)) {
