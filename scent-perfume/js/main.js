...
function initMobileMenu() {
  ...
  const handle  = drawer ? drawer.querySelector('.nav__drawer-handle') : null;
  ...
  // ВАЖНО: только click. click на мобиле срабатывает по тапу,
  // а одновременный touchend+click давал двойное переключение
  // (открыл и тут же закрыл) — из-за этого меню "не работало".
  burger.addEventListener('click', toggleDrawer);
  overlay.addEventListener('click', closeDrawer);

  // Ссылки в drawer работают как обычные ссылки (нативная навигация).
  // Никаких preventDefault/touchend — именно они ломали все ссылки кроме первой.
  drawer.querySelectorAll('.nav__drawer-link').forEach(link => {
    link.addEventListener('click', () => {
      // закрываем меню; навигация по ссылке произойдёт сама
      closeDrawer();
    });
  });

  // Свайп вниз для закрытия — ТОЛЬКО на полоске-ручке (не на всём drawer,
  // иначе перехватывал тапы по ссылкам).
  if (handle) {
    let startY = 0;
...
/* ─── EVENT DELEGATION для карточек (Android fix) ─────────── */
function initProductGridEvents() {
  const grid = document.getElementById('productsGrid');
  if (!grid) return;

  // Remove old listener if any
  grid.removeEventListener('click', productGridClick);
  grid.removeEventListener('touchend', productGridTouch);

  grid.addEventListener('click', productGridClick);
}

function productGridClick(e) {
  ...
  if (btn) {
    ...
    if (action === 'volume') openVolumeSelector(id);
    if (action === 'wa') handleOrder(id);
    return;
  }
  // Click on card itself — open modal or volume selector
  const card = e.target.closest('.product__card[data-id]');
  if (card && !e.target.closest('button')) {
    const id = parseInt(card.dataset.id);
    openVolumeSelector(id);
  }
}

function productGridTouch(e) {
  // handled by click on Android
}
